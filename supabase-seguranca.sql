...
CREATE POLICY "galeria_select" ON storage.objects FOR SELECT
  TO authenticated USING (bucket_id = 'galeria');
CREATE POLICY "galeria_insert" ON storage.objects FOR INSERT
  TO authenticated WITH CHECK (bucket_id = 'galeria');
CREATE POLICY "galeria_delete" ON storage.objects FOR DELETE
  TO authenticated USING (bucket_id = 'galeria');
-- NOTA: essas policies liberam o bucket a qualquer usuario autenticado (sem
-- isolar por clinica no caminho do arquivo). Para isolamento por pasta, refine
-- usando (storage.foldername(name))[1] = clinica_id. Mantido simples aqui.
